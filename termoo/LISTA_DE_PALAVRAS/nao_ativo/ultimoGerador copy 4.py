...
def carrega_palavra_secreta():
    url = "https://www.invertexto.com/gerador-palavras-aleatorias"
    # Parâmetros do formulário para gerar palavras:
    # 'tipo' = 'todas' (todas as palavras)
    # 'qtde' = 1 (quantidade de palavras)
    # 'letras' = '' (qualquer quantidade)
    # 'comeca' = '' (sem filtro)
    # 'termina' = '' (sem filtro)
    dados = {
        'tipo': 'todas',
    ...
    try:
        with request_semaphore:
            ...
            if resposta.status_code == 200:
                ...
                # As palavras geradas aparecem dentro de um <textarea id="resultado">
                palavras = elemento.xpath('//textarea[@id="resultado"]/text()')
                if palavras:
                    palavra_secreta = palavras[0].strip().split('\n')[0]  # pega a primeira palavra
                    backoff_config.decrease_delay()
                    return palavra_secreta
                else:
                    raise Exception("Palavra não encontrada na resposta")
            else:
