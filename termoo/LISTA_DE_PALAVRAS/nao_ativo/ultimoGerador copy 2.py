        ...
        with self.lock:
            ...
            if current_time - self.last_increase_time >= self.min_time_between_increases:
                if self.current_delay == 0:
                    self.current_delay = 0.1  # Começa com meio segundo
                else:
...
def carrega_palavra_secreta():
    ...
    try:
        ...
        if novo_delay > backoff_config.get_current_delay() / 1.5:
...
# arquivo_lock = threading.Lock()
# # Semáforo para controlar o número de requisições simultâneas
# request_semaphore = threading.Semaphore(20)  # Aumentado para 20 requisições simultâneas

# # Configurações de backoff
# class BackoffConfig:
#     def __init__(self):
#         self.min_delay = 0  # Delay mínimo inicial
#         self.max_delay = 10  # Reduzido para 10 segundos para limitar espera máxima
#         self.current_delay = self.min_delay
#         self.backoff_factor = 1.2  # Aumento mais gradual do delay
#         self.success_streak = 0
#         self.required_successes = 2  # Menos sucessos para reduzir delay
#         self.lock = threading.Lock()
#         self.last_increase_time = time.time()
#         self.min_time_between_increases = 0.5  # Reduzido para 0.5 segundos
...
#             if current_time - self.last_increase_time >= self.min_time_between_increases:
#                 if self.current_delay == 0:
#                     self.current_delay = 0.05  # Começa com 50ms
#                 else:
#                     self.current_delay = min(self.current_delay * self.backoff_factor, self.max_delay)
#                 self.last_increase_time = current_time
#                 self.success_streak = 0
...
#     try:
#         with request_semaphore:
#             resposta = requests.get(url, timeout=5)  # Timeout para evitar travar
#             if resposta.status_code == 200:
#                 elemento = html.fromstring(resposta.content)
#                 textos = elemento.xpath('//div//text()')
#                 textos = [t.strip() for t in textos if t.strip()]
...
#             delay_atual = backoff_config.get_current_delay()
#             if delay_atual > 0:
#                 variacao = random.uniform(-0.05 * delay_atual, 0.05 * delay_atual)  # Variação pequena para estabilidade
#                 delay_final = max(delay_atual + variacao, 0)
#             else:
#                 delay_final = 0
...
#             if delay_final > 0:
#                 time.sleep(delay_final)
#             # Se delay_final == 0, não dorme nada e faz próxima requisição imediatamente
...
# def coleta_palavras_paralela(num_threads=40):
#     threads = []
#     print(f"Iniciando coleta com {num_threads} threads paralelas...")
#     print("Pressione Ctrl+C para parar.")
#     try:
...
#             threads.append(thread)
#             thread.start()
#             # Removido o sleep para iniciar threads o mais rápido possível
#         while True:
#             time.sleep(1)
#     except KeyboardInterrupt:
#         print("\nEncerrando coleta de palavras...")
...
# if __name__ == "__main__":
#     NUM_THREADS = 40  # Aumentado para 40 threads
#     coleta_palavras_paralela(NUM_THREADS)
