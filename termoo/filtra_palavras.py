...
def escolhe_palavras(palavras, mais_comuns=True,porcentagem_do_topo=0.8):
    ...
    wordfreq_normalizado = set(normalizar(p) for p in top_n_list('pt', n_de_escolhidos))

    if mais_comuns:
        palavras_escolhidas = [palavra for palavra in palavras_sem_espaco if palavra in wordfreq_normalizado]
    else:
        palavras_escolhidas = [palavra for palavra in palavras_sem_espaco if palavra not in wordfreq_normalizado]
