...
from lista_unificada_20_04_2025__20_32 import palavras2
import requests
import random
from datetime import datetime
import requests
...
class Termoo():
    def __init__(self):
        ...
        self.possiveisChaves=palavras2
    ...
    def colunai(self, chave, iColuna):
        ...
        if chave in self.palavrasAcertadas:
            # Mostra a palavra acertada em verde com padding
            palavra_pintada = " ".join([self.verde + letra + self.resetar_cor for letra in chave])
            linhas = [palavra_pintada] * self.nChutesTotais
            return "\n".join(linhas)
    ...
    def todasAsColunas(self):
        ...
        for iColuna in range(self.nPalavras):
            ...
            if chave_atual in self.palavrasAcertadas:
                texto_coluna = self.colunai(chave_atual, iColuna).split('\n')
                colunas[iColuna] = texto_coluna
            else:
                linhas_coluna = []
                for chute in self.palavrasChutadas:
